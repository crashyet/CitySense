...
export default function Simulator() {
  ...
  const handleSaveScenario = () => {
    ...
    const newScenario = {
      ...
      values: {
        electricVehicle: electricVehicle ?? 0,
        greenSpaces: greenSpaces ?? 0,
        wasteSystem: wasteSystem ?? 0,
        solarPanel: solarPanel ?? 0,
      },
    };
  ...
  {savedScenarios.length === 0 && (
    <p className="text-gray-400 text-center">Belum ada skenario tersimpan.</p>
  )}
  ...
  {savedScenarios.map((s) => (
    ...
    >
      ...
      <div className="grid grid-cols-2 mt-3 text-sm text-gray-300 gap-y-1">
        <p>
          EV:
          <span className="font-semibold text-white"> {s.values?.electricVehicle}%</span>
        </p>
        <p>
          Taman Hijau:
          <span className="font-semibold text-white"> {s.values?.greenSpaces}%</span>
        </p>

        <p>
          Pengelolaan Limbah:
          <span className="font-semibold text-white"> {s.values?.wasteSystem}%</span>
        </p>
        <p>
          Panel Surya:
          <span className="font-semibold text-white"> {s.values?.solarPanel}%</span>
        </p>
      </div>
